...
而 Charles 的自签名证书中的域名，肯定跟我们自己APP的域名是不匹配的，那么客户端的证书校验肯定没办法通过？这也进一步降低了中间人攻击的风险。
...
正常情况下，Charles 等抓包工具，肯定是无法对 HTTPS 协议的网络请求进行抓包的，因为第一步证书的校验就无法通过。为了在测试环境中，能够正常的使用 Charles 抓包工具，我们就需要进行如下操作。
