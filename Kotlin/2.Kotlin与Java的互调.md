...
Kotlin在设计之初，就考虑了与Java的互操作性。因此Java和Kotlin是可以很方便的进行互相调用的。下面就详细介绍一下Java与Kotlin在互相调用的时候，所需要注意的一些细节。
...
为了避免调用Java代码可能产生的空指针，我们可以把Java对应的平台类型变量，赋值给一个Kotlin变量，从而使用Kotlin断言机制，预防空指针，如：

```kotlin
 fun test() {
       val user = User()
   			
   // 先把变量赋值给一个Kotlin变量，这样在调用对象的时候，Kotlin会检测是否为空，如果为空，就不调用对象方法
        val userName: String? = user.name // // 允许，没有问题
   			val userName2:String =user.name // 允许，运行时可能失败，还是可能会发生空指针，与直接调用没有本质区别
