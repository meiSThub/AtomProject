...
在讲Java运行时内存之前，先来看看类是如何被加载到内存中来的。类的加载时通过ClassLoader加载到内存中的。下面就先了解一下Android中，ClassLoader有哪些类型。
...
从打印信息可以看出，PathClassLoader加载的dex文件路径是：**"/data/app/com.mei.memoryopt-mjnAExPuozZ0Se4OXdpW0A==/base.apk"**，

父类是：**BootClassLoader**，是在构造函数中传入的。

**DexPathList**在是BaseDexClassLoader的构造方法中创建的，里面存储了dex相关的文件路径。

继承关系类图如下：

![WeChatWorkScreenshot_5d8b3068-20d0-490c-af15-07eaac83c50f](../../image/WeChatWorkScreenshot_5d8b3068-20d0-490c-af15-07eaac83c50f.png)
...
ClassLoader的加载方法为loadClass方法，被定义在抽象ClassLoader中，如下所示：
...
在loadClass方法中，首先会检查类是否加载过，没有加载就采用了一个双亲委托的模式去加载。

双亲委托：即先不是自身去查找而是委托给父加载器进行查找，这样依次进行递归，直到委托到最顶层的ClassLoader，如果最顶层ClassLoader找到了，就直接返回，如果没有找到，则继续依次向下查找，如果还是没有找到则最后会交由自身去查找。流程如下图：

![image-20200722155307346](../../image/image-20200722155307346.png)
...
public Class<?> findClass(String name, List<Throwable> suppressed) {
  // 这里每一个Element对象，都代表一个dex文件，如果App采用了MultiDex技术，则会有多个Dex文件
...
​		现在**主流的虚拟机的垃圾收集器都采用分代收集算法**。Java 堆区基于分代的概念，分为新生代（Young Generation）和老年代（Tenured Generation），其中新生代再细分为Eden空间、From Survivor空间和To Survivor空间。因为Eden空间中的大多数对象生命周期很短，所以新生代的空间划分并不是均分的，HotSpot虚拟机默认Eden空间和两个Survivor空间的所占的比例为8：1。Java堆区的空间划分如下图所示。
...
* Dalvik虚拟机
  ...
  Dalvik虚拟机主要使用标记清除算法，也可以选择使用拷贝算法。这取决于编译时期：http://androidxref.com/4.4_r1/xref/dalvik/vm/Dvm.mk

  ![image-20200719212118439](../../image/image-20200719212118439.png)

*  ART虚拟机：

  ART 是在 Android 4.4 中引入的，在Android4.4默认使用的是Dalvik虚拟机，而在Android 5.0 及更高版本的默认使用ART虚拟机。至此google已不再继续维护和提供 Dalvik 运行时，现在 ART 采用了其字节码格式。ART 有多个不同的 GC 方案，这些方案包括运行不同垃圾回收器。默认方案是 CMS。
...
![image-20200722165920292](../../image/image-20200722165920292.png)
...
* 并发标记（CMS concurrent mark）：
  ...
  进行GC Roots 追踪的过程，刚才产生的集合中标记出存活对象，应用程序也在运行；
...
参考文档：https://source.android.com/devices/tech/dalvik/gc-debug?hl=zh-cn



通过对CMS垃圾收集器的了解，在垃圾回收的过程中，不可避免的会暂停其它的线程，如果某个时间内频繁的进行内存的分配，造成内存不足并引发频繁的GC，就会造成内存的抖动，也就会造成UI的卡顿。
...
2. **内存抖动：**
   ...
   ![image-20200720173654899](../../image/image-20200720173654899.png)
