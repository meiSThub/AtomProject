...
* 监听 Activity 的生命周期
* 在 onDestroy 的时候，把Activity对象存入弱引用(KeyedWeakReference)中，弱引用如引用队列(RefrenceQueue)关联，并启动后台进程去检测
* 一段时间之后，从 RefrenceQueue 读取，若读取不到相应 activity 的 KeyedWeakReference，有可能发生泄露了，这个时候，再触发发 gc，一段时间之后，再去读取，若在从 RefrenceQueue 还是读取不到相应 activity 的 refrence，可以断定是发生内存泄露了
