...
从上的分析就可以知道，**Fragment 之间为什么可以共享 Activity 的 ViewModel 对象，而不会重新创建 ViewModel 对象，因为ViewModel 在创建之后，就缓存到了 ViewModelStore 中，当下次通过 ViewModelProvider 获取相同类的 ViewModel 对象的时候，就直接从 ViewModelStore 缓存中获取到 ViewModel 对象了，所以才不会重新状态 ViewModel 对象。**
